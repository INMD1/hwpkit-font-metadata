...
function reportSummary(kind: string, output: string, errorCount: number): void {
  if (output === "-") return;
...
export function parseMerge(args: string[]): MergeCliOptions {
  ...
  // No paths given: merge whatever "analyze" already wrote to the default result folder.
...
export function mergeFontCatalogs(inputs: readonly MergeInput[]): {
  ...
  return { catalog: createCatalog(fonts, errors), stats: { catalogFiles, skippedFiles, duplicateFontCount } };
}
...
async function runMerge(options: MergeCliOptions): Promise<void> {
  ...
  if (stats.duplicateFontCount > 0) parts.push(`, ${stats.duplicateFontCount} duplicate(s) skipped`);
  if (stats.skippedFiles.length > 0) parts.push(`, ${stats.skippedFiles.length} non-catalog file(s) skipped`);
  parts.push(")\n");

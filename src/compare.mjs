 ...
 */
export const DEFAULT_ADJUSTMENT_LIMITS = Object.freeze({
  hwp: Object.freeze({
    ratio: Object.freeze([95, 105]),
    spacing: Object.freeze([-2, 2]),
  }),
  hwpx: Object.freeze({
    ratio: Object.freeze([95, 105]),
    spacing: Object.freeze([-2, 2]),
...
const WIDTH_KEYS = Object.freeze(Object.keys(DEFAULT_WIDTH_METRIC_WEIGHTS));
const VERTICAL_KEYS = Object.freeze(Object.keys(DEFAULT_VERTICAL_METRIC_WEIGHTS));
const STYLE_KEYS = Object.freeze(Object.keys(DEFAULT_STYLE_METRIC_WEIGHTS));
...
function validateNonNegativeWeights(weights, label) {
...
function normalizeLimit(limit, fallback, label) {
...
function normalizeOptions(options = {}) {
  ...
  const requestedLimits = options.adjustmentLimits ?? {};
...
function normalizeCoverageRatio(set) {
  ...
  if (!isFiniteNumber(ratio) && isFiniteNumber(set.mapped) && isFiniteNumber(set.required)) {
    ratio = set.required === 0 ? 1 : set.mapped / set.required;
  }
...
function coverageSetDetails(set) {
 ...
 */
export function checkCoverageCompatibility(reference, candidate, options = {}) {
...
function checkCoverageInternal(reference, candidate, options) {
...
function groupMean(profile, aliases) {
...
function averageGroupMeans(profile, aliases) {
  const groups = profile.metrics?.advance?.groups ?? {};
...
function bodyTextSampleMean(profile) {
  const samples = Array.isArray(profile.layout?.samples) ? profile.layout.samples : [];
  const preferred = samples.find(
    (sample) => sample?.complete && /body|paragraph|korean/i.test(String(sample.id ?? '')),
  );
  if (isFiniteNumber(preferred?.advancePerCodePointEm)) {
    return preferred.advancePerCodePointEm;
  }
  const usable = samples
    .filter((sample) => sample?.complete && isFiniteNumber(sample.advancePerCodePointEm))
    .map((sample) => sample.advancePerCodePointEm);
  if (usable.length === 0) return undefined;
  return usable.reduce((sum, value) => sum + value, 0) / usable.length;
}

function extractWidthModel(profile) {
...
function extractSpaceAdvance(profile, widthModel = extractWidthModel(profile)) {
  if (isFiniteNumber(widthModel.spaceEm)) return widthModel.spaceEm;
  const spaces = profile.metrics?.advance?.spaces ?? {};
  const entries = Object.values(spaces);
...
function extractLineModel(profile) {
...
function fixedMetricDistance(referenceValue, candidateValue) {
...
function signedDelta(referenceValue, candidateValue) {
  const referenceMagnitude = magnitude(referenceValue);
  const candidateMagnitude = magnitude(candidateValue);
...
function metricComponent(referenceValues, candidateValues, metricWeights, options) {
...
function styleComponent(referenceStyle, candidateStyle, options) {
  let weightedDistance = 0;
  let totalWeight = 0;
  const deltas = {};
  const missing = [];
  const compared = [];
  const fixedScales = { weightClass: 900, widthClass: 8 };
  ...
  for (const key of STYLE_KEYS) {
    ...
    if (candidateValue === undefined) {
      weightedDistance += weight * options.missingMetricPenalty;
      missing.push(key);
      deltas[key] = { reference: referenceValue, candidate: null, relative: null };
      continue;
    }

    const distance = key === 'italic'
      ? (candidateValue === referenceValue ? 0 : 1)
      : Math.abs(candidateValue - referenceValue) / fixedScales[key];
...
function weightedTotal(components, componentWeights) {
  let weightedDistance = 0;
  let totalWeight = 0;
...
function primaryWidth(referenceWidths, candidateWidths) {
 ...
 * replacement font.
 */
export function computeFormatAdjustments(reference, candidate, options = {}) {
  validateProfile(reference, 'reference');
  validateProfile(candidate, 'candidate');
  const normalized = normalizeOptions(options);
...
function computeFormatAdjustmentsInternal(reference, candidate, options) {
...
function compareInternal(reference, candidate, options) {
...
export function compareFontProfiles(reference, candidate, options = {}) {
  validateProfile(reference, 'reference');
  validateProfile(candidate, 'candidate');
 ...
 */
export function rankFontCandidates(reference, candidates, options = {}) {
  validateProfile(reference, 'reference');
